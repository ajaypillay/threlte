<script lang="ts">
      ...
      toolbars: {
        global: ((set, studio) => {
          ...
          const remove = {
            ...
            onClick: () => {
              if (!sheetObject) return
              removeElement(studio, entities, sheetObject)
            }
          }
  ...
  const getElementsExtension = (): IExtension => {
    return {
      ...
      toolbars: {
        global: ((set, studio) => {
          const addHandler = Object.values(elementConfigurations).map((element) => {
            return {
              type: 'Icon',
              ...
              onClick: () => {
                ...
                if (!isISheetObject(currentSelection)) {
                  // add
                  const entitiesValueBefore = entities[element.name] ?? ''
                  const newId = createEntityId()
                  ...
                  const entitiesValueBeforeOfOldElement = entities[elementName]
                  ...
                  const entitiesValueBeforeOfNewElement = entities[element.name] ?? ''
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Backspace' && e.getModifierState('Control')) {
      ...
      e.preventDefault()
      removeElement(studio.current, entities, sheetObject)
    }
  }
</script>
